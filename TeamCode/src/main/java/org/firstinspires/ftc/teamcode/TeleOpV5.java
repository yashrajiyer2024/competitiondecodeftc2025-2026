...
@TeleOp(name = "V5-ButtonPad", group = "Competition")
public class TeleOpV5 extends LinearOpMode {
    ...
    private double HIGH_RPM_STEP = 25;
    ...
    private double BACKUP_SPEED = 0.5;
    private long BACKUP_DURATION = 1000; // milliseconds
    ...
    private void handleBackup() {
        if (gamepad1.x) {
            if (!isBackingUp) {
                // Start time-based backup
                frontLeft.setVelocity(-DRIVE_TICKS_PER_SEC * BACKUP_SPEED);
                frontRight.setVelocity(-DRIVE_TICKS_PER_SEC * BACKUP_SPEED);
                backLeft.setVelocity(-DRIVE_TICKS_PER_SEC * BACKUP_SPEED);
                backRight.setVelocity(-DRIVE_TICKS_PER_SEC * BACKUP_SPEED);
